...
const App: React.FC = () => {
  ...
  const [editingDraft, setEditingDraft] = useState<
    { id: string; original: FeatureCollection; geojson: FeatureCollection } | null
  >(null);

  const mapLayers = useMemo(() => {
    if (!editingDraft) return layers;
    return layers.map(l =>
      l.id === editingDraft.id ? { ...l, geojson: editingDraft.geojson } : l
    );
  }, [layers, editingDraft]);
  ...
  const handleToggleEditLayer = useCallback((id: string) => {
    ...
    setEditingDraft({ id, original: snapshot, geojson: snapshot });
  ...
  const handleUpdateLayerGeojson = useCallback(
    (id: string, geojson: FeatureCollection) => {
      if (editingDraft && editingDraft.id === id) {
        setEditingDraft({ ...editingDraft, geojson });
      } else {
        setLayers(prev =>
          prev.map(layer => (layer.id === id ? { ...layer, geojson } : layer))
        );
      }
  ...
  const handleDiscardEditLayer = useCallback(() => {
    if (!editingDraft) return;
    setLayers(prev => prev.map(l => l.id === editingDraft.id ? { ...l, geojson: editingDraft.original } : l));
