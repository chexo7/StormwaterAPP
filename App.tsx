...
const App: React.FC = () => {
  ...
  const handleExportSWMM = useCallback(async () => {
    ...
    const nodeLookup = new Map<
      string,
      { id: string; raw: string; coord: [number, number]; invert: number }
    >();
    ...
    if (jLayer) {
      ...
      jLayer.geojson.features.forEach((f, i) => {
        ...
        const node = { id, raw, coord, invert };
        nodes.push(node);
        nodeLookup.set(raw, node);
      });
    }
    ...
    if (pLayer && nodes.length) {
      ...
      pLayer.geojson.features.forEach((f, i) => {
        ...
        if (direction) {
          const parts = direction.split(/\s+to\s+/i).map((s) => s.trim());
          if (parts.length === 2) {
            from = nodeLookup.get(parts[0]);
            to = nodeLookup.get(parts[1]);
          }
        }
        const start = project.forward(coords[0] as [number, number]);
        const end = project.forward(coords[coords.length - 1] as [number, number]);
        if (!from) from = findNearestNode(start);
        if (!to) to = findNearestNode(end);
