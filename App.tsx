...
<html><head><title>3D Pipe Network</title>
<style>html,body{margin:0;height:100%;overflow:hidden;background:#000}#center{position:absolute;top:10px;left:10px;z-index:1}</style>
</head><body>
...
const xs=[],ys=[],zs=[];
data.nodes.forEach(n=>{xs.push(n.x);ys.push(n.y);zs.push(n.ground,n.invOut);});
...
const camera=new THREE.PerspectiveCamera(60,window.innerWidth/window.innerHeight,0.1,size*10);
const renderer=new THREE.WebGLRenderer({antialias:true});
renderer.setSize(window.innerWidth,window.innerHeight);
renderer.setClearColor(0x000000,1);
document.body.appendChild(renderer.domElement);
const controls=new THREE.OrbitControls(camera,renderer.domElement);
function reset(){camera.position.set(0,-size*1.5,size*1.5);controls.target.set(0,0,0);controls.update();}
reset();
data.nodes.forEach(n=>{const s=new THREE.Vector3(n.x,n.y,n.invOut);const e=new THREE.Vector3(n.x,n.y,n.ground);const dir=new THREE.Vector3().subVectors(e,s);const len=dir.length();if(len>0){const g=new THREE.CylinderGeometry(n.diam/2,n.diam/2,len,16,false);const m=new THREE.MeshBasicMaterial({color:0xffffff});const mesh=new THREE.Mesh(g,m);const axis=new THREE.Vector3(0,1,0);mesh.quaternion.setFromUnitVectors(axis,dir.clone().normalize());mesh.position.copy(s.clone().add(dir.multiplyScalar(0.5)));scene.add(mesh);}});
data.pipes.forEach(p=>{const s=new THREE.Vector3(p.start.x,p.start.y,p.start.z);const e=new THREE.Vector3(p.end.x,p.end.y,p.end.z);const dir=new THREE.Vector3().subVectors(e,s);const len=dir.length();if(len>0){const g=new THREE.CylinderGeometry(p.diam/2,p.diam/2,len,8,false);const m=new THREE.MeshBasicMaterial({color:0xffffff});const mesh=new THREE.Mesh(g,m);const axis=new THREE.Vector3(0,1,0);mesh.quaternion.setFromUnitVectors(axis,dir.clone().normalize());mesh.position.copy(s.clone().add(dir.multiplyScalar(0.5)));scene.add(mesh);}});
