...
type HydroCADSubcatchmentGroup = {
  key: string;
  parent: string;
  subareaName: string;
  complement: boolean;
...
const sanitizeHydroCADId = (value: string): string =>
  value.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 31);

const aggregateOverlayForHydroCAD = (
  features: Feature[]
): HydroCADSubcatchmentGroup[] => {
  ...
  features.forEach(feature => {
    ...
    const parentRaw = props[SUBAREA_PARENT_ATTR] ?? props[DA_NAME_ATTR];
    const parent = formatDischargePointName(parentRaw);
    const subareaRaw = props[SUBAREA_NAME_ATTR];
    const subareaName = subareaRaw == null ? '' : String(subareaRaw).trim();

    if (!parent || !subareaName) return;
    ...
    entry.totalArea += areaValue;

    const mapKey = `${cnValue}|${desc ?? ''}`;
    const existing = entry.areaMap.get(mapKey);
    if (existing) {
      existing.area += areaValue;
    } else {
      entry.areaMap.set(mapKey, { area: areaValue, cn: cnValue, desc });
    }
  });

  const subcatchments = Array.from(grouped.entries()).map(([key, value]) => ({
    key,
    parent: value.parent,
    subareaName: value.subareaName,
    complement: value.complement,
    totalAreaSqft: Number(value.totalArea.toFixed(2)),
    areas: Array.from(value.areaMap.values()).map(item => ({
  ...
  subcatchments.sort((a, b) => {
    const parentCmp = a.parent.localeCompare(b.parent);
    if (parentCmp !== 0) return parentCmp;
    return a.subareaName.localeCompare(b.subareaName);
  });
...
const buildHydroCADContent = (
  subcatchments: HydroCADSubcatchmentGroup[],
  projectName: string
): string => {
  ...
  subcatchments.forEach((subcatchment, index) => {
    const baseId = sanitizeHydroCADId(
      `${subcatchment.parent}-${subcatchment.subareaName}`
    );
    const fallbackId = `SUB${index + 1}`;
    let nodeId = baseId || fallbackId;
    while (usedIds.has(nodeId)) {
      nodeId = sanitizeHydroCADId(`${baseId || fallbackId}-${usedIds.size + 1}`) || fallbackId;
      if (!usedIds.has(nodeId)) break;
    }
    usedIds.add(nodeId);

    const displayNameParts = [subcatchment.parent, subcatchment.subareaName].filter(Boolean);
    const nodeName = displayNameParts.join(' - ') || nodeId;
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (overlay.length > 0) {
        ...
        try {
          ...
          addLog(
            `Archivo HydroCAD generado utilizando el método SCS dentro del Overall Drainage Area. Subcatchments: ${subcatchments.length}.`
          );
        } catch (error) {
  ...
  const handleExportHydroCAD = useCallback(() => {
    ...
    const subcatchments = aggregateOverlayForHydroCAD(overlayLayer.geojson.features);
    if (subcatchments.length === 0) {
