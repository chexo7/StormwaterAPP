...
const HYDRO_SUBCATCH_ATTR = 'HYDRO_ID';
...
const buildHydroSubcatchKey = (
  parent: string | null | undefined,
  name: string | null | undefined
): string => {
  const parentKey = parent ? String(parent).trim().toLowerCase() : '';
  const nameKey = name ? String(name).trim().toLowerCase() : '';
  return `${parentKey}||${nameKey}`;
};

const getHydroSubcatchKeyFromProps = (
  props: Record<string, unknown> | null | undefined
): string | null => {
  if (!props) return null;
  const explicit = (props as any)[HYDRO_SUBCATCH_ATTR];
  if (explicit != null && String(explicit).trim() !== '') {
    return String(explicit);
  }
  const parent = formatDischargePointName(
    (props as any)[SUBAREA_PARENT_ATTR] ?? (props as any)[DA_NAME_ATTR]
  );
  const subNameRaw = (props as any)[SUBAREA_NAME_ATTR];
  const subName = subNameRaw == null ? '' : String(subNameRaw).trim();
  const key = buildHydroSubcatchKey(parent, subName);
  return key === '||' ? null : key;
};

const sanitizeHydroCADNumber = (value: string): string => {
  const cleaned = value.replace(/[^a-zA-Z0-9]/g, '').slice(0, 24);
  return cleaned || 'SUB';
};

const ensureUniqueHydroCADNumber = (
  base: string,
  used: Set<string>
): string => {
  const safeBase = base || 'SUB';
...
type HydroCADNode = {
  ...
  weightedCn: number | null;
};

type HydroCADSubareaStats = {
  name: string;
  parent: string | null;
  isComplement: boolean;
  totalAreaSqft: number;
...
const aggregateOverlayForHydroCAD = (
  ...
  nodes: HydroCADNode[];
  statsByKey: Map<string, HydroCADSubareaStats>;
  missingCnKeys: Set<string>;
} => {
  const grouped = new Map<
    string,
    {
      ...
      parent: string | null;
      isComplement: boolean;
      areaMap: Map<string, HydroCADAreaGroup>;
      totalAreaSqft: number;
      totalAreaAcres: number;
      weightedCnSum: number;
    }
  >();
  const missingCnKeys = new Set<string>();
  ...
  features.forEach(feature => {
    ...
    const parent =
      formatDischargePointName(
        props[SUBAREA_PARENT_ATTR] ?? props[DA_NAME_ATTR]
      ) || null;
    const subNameRaw = props[SUBAREA_NAME_ATTR];
    const subName = subNameRaw == null ? '' : String(subNameRaw).trim();
    const keyFromProps = getHydroSubcatchKeyFromProps(props);
    if (!keyFromProps) return;

    const cnRaw = props?.CN;
    const cnValue = Number(cnRaw);
    const hasCn = Number.isFinite(cnValue);

    const areaProp = props?.AREA_SF;
    let areaSqft = Number(areaProp);
    ...
    if (!grouped.has(keyFromProps)) {
      grouped.set(keyFromProps, {
        name: subName || (parent ? `${parent} Subarea` : 'Subarea'),
        parent,
        isComplement: Boolean(props[COMPLEMENT_FLAG_ATTR]),
        areaMap: new Map(),
        totalAreaSqft: 0,
        totalAreaAcres: 0,
        weightedCnSum: 0,
    ...
    const entry = grouped.get(keyFromProps)!;

    if (areaSqft > 0) {
      entry.totalAreaSqft += areaSqft;
      entry.totalAreaAcres += areaSqft * SQFT_TO_ACRES;
      if (hasCn) {
        entry.weightedCnSum += areaSqft * cnValue;
      } else {
        missingCnKeys.add(keyFromProps);
      }
    }

    if (!hasCn || areaSqft <= 0) return;

    const lcRaw = props?.LAND_COVER;
    const lcName = lcRaw == null ? null : String(lcRaw);
    const hsgRaw = props?.HSG;
    const hsg = hsgRaw == null ? null : String(hsgRaw).toUpperCase();
    const desc = lcName ? `${lcName}${hsg ? `, HSG ${hsg}` : ''}` : undefined;
    const areaKey = `${cnValue}|${desc ?? ''}`;
    const existing = entry.areaMap.get(areaKey);
    if (existing) {
      existing.area += areaSqft;
    } else {
      entry.areaMap.set(areaKey, { area: areaSqft, cn: cnValue, desc });
    }
  });

  const usedNumbers = new Set<string>();
  const nodes: HydroCADNode[] = [];
  const statsByKey = new Map<string, HydroCADSubareaStats>();

  grouped.forEach((entry, key) => {
    const areas = Array.from(entry.areaMap.values()).map(item => ({
    ...
    const weightedCn =
      entry.totalAreaSqft > 0 && entry.weightedCnSum > 0
        ? Number((entry.weightedCnSum / entry.totalAreaSqft).toFixed(2))
        : null;

    const baseNumber = sanitizeHydroCADNumber(entry.name || entry.parent || 'Sub');
    const number = ensureUniqueHydroCADNumber(baseNumber, usedNumbers);

    nodes.push({
      key,
      number,
      name: entry.name || number,
      parent: entry.parent,
      isComplement: entry.isComplement,
      areas,
      totalAreaSqft: Number(entry.totalAreaSqft.toFixed(2)),
      weightedCn,
    });
    ...
    statsByKey.set(key, {
      ...
      isComplement: entry.isComplement,
      totalAreaSqft: Number(entry.totalAreaSqft.toFixed(2)),
      totalAreaAcres: Number(entry.totalAreaAcres.toFixed(4)),
      weightedCn,
    });
  });
  ...
  return { nodes, statsByKey, missingCnKeys };
};
...
const buildHydroCADContent = (
  ...
  let y = 0;
  nodes.forEach(node => {
    const displayName = node.isComplement ? `${node.name} (Complemento)` : node.name;
    content += `\n[NODE]\nNumber=${node.number}\nType=Subcat\nName=${displayName}\nXYPos=0 ${y}\n`;
    const descParts: string[] = [];
    if (node.parent) descParts.push(`Parent: ${node.parent}`);
    if (node.isComplement) descParts.push('Complemento');
    if (descParts.length > 0) {
      content += `Desc=${descParts.join(' | ')}\n`;
    }
...
const App: React.FC = () => {
    ...
    const statuses: ScsLayerStatus[] = [
      ...
      },
      {
        ...
        description: !soilsLayer
          ...
          : 'Asigna manualmente un HSG (A/B/C/D) a cada polígono.',
      },
    ];
  ...
  const computeEnabled = lodValid && scsReady;
  ...
  const runCompute = useCallback(async () => {
    ...
    try {
      ...
      const expectedHydroKeys = new Map<string, { name: string; parent: string }>();
      ...
      groupedDas.forEach(({ features, baseProps }, daName) => {
        ...
        relatedSubareas.forEach((subFeature, subIndex) => {
          ...
          const hydroKey = buildHydroSubcatchKey(daName, subName);
          ...
          expectedHydroKeys.set(hydroKey, { name: subName, parent: daName });
        });
        ...
        if (remainderGeom) {
          flattenEach(remainderGeom as any, rem => {
            ...
            const complementName = `${daName} - Complement`;
            const complementKey = buildHydroSubcatchKey(daName, complementName);
            const complementProps = {
              ...baseProps,
              [DA_NAME_ATTR]: daName,
              [SUBAREA_PARENT_ATTR]: daName,
              [SUBAREA_NAME_ATTR]: complementName,
              [COMPLEMENT_FLAG_ATTR]: true,
              [SUBAREA_AREA_ATTR]: Number((remArea * SQM_TO_ACRES).toFixed(6)),
              [HYDRO_SUBCATCH_ATTR]: complementKey,
            };
      ...
      if (overlay.length > 0) {
        ...
        try {
          const { nodes, statsByKey, missingCnKeys } = aggregateOverlayForHydroCAD(overlay);

          const missingExpected = Array.from(expectedHydroKeys.entries()).filter(
            ([key]) => !statsByKey.has(key)
          );
          if (missingExpected.length > 0) {
            const names = missingExpected.map(([, info]) => info.name || '(sin nombre)');
            throw new Error(
              `Faltan resultados del método SCS para las subáreas: ${names.join(', ')}`
            );
          }

          const missingCnForExpected = Array.from(missingCnKeys).filter(key =>
            expectedHydroKeys.has(key)
          );
          if (missingCnForExpected.length > 0) {
            const names = missingCnForExpected
              .map(key => expectedHydroKeys.get(key)?.name || '(sin nombre)')
              .join(', ');
            throw new Error(
              `No se pudo calcular el número CN para: ${names}. Revisa Land Cover y WSS.`
            );
          }

          if (nodes.length === 0) {
            throw new Error('No se generaron nodos HydroCAD a partir del overlay.');
          }

          const enrichedProcessed = processedSubareas.map(feature => {
            const props = { ...(feature.properties || {}) };
            const key = getHydroSubcatchKeyFromProps(props) || '';
            if (statsByKey.has(key)) {
              const stats = statsByKey.get(key)!;
              if (stats.weightedCn != null) {
                props.CN = stats.weightedCn;
              }
              props.AREA_SF = stats.totalAreaSqft;
              props.AREA_AC = stats.totalAreaAcres;
            }
            return { ...feature, properties: props };
          });

          const processedLayerIndex = resultLayers.findIndex(
            layer => layer.name === PROCESSED_SUBAREA_LAYER_NAME
          );
          if (processedLayerIndex !== -1) {
            const processedLayer = resultLayers[processedLayerIndex];
            resultLayers[processedLayerIndex] = {
              ...processedLayer,
              geojson: {
                ...(processedLayer.geojson as FeatureCollection),
                features: enrichedProcessed as Feature[],
              },
            };
          }

          const subareaNodeCount = nodes.filter(node => !node.isComplement).length;
          if (subareaNodeCount !== expectedHydroKeys.size) {
            throw new Error(
              'El número de subcuencas generadas no coincide con las subáreas originales.'
            );
          }

          const content = buildHydroCADContent(nodes, projectName);
          triggerHydroCADDownload(content, projectName, projectVersion);
          setComputeTasks(prev =>
            prev.map(t => (t.id === 'hydrocad' ? { ...t, status: 'success' } : t))
          );
          const complementNodeCount = nodes.filter(node => node.isComplement).length;
          const summaryPieces = [`${subareaNodeCount} subcuencas`];
          if (complementNodeCount > 0) {
            summaryPieces.push(`${complementNodeCount} complementos`);
          }
          addLog(
            `Archivo HydroCAD generado utilizando el método SCS dentro del Overall Drainage Area -> ${summaryPieces.join(', ')}`
          );
        } catch (error) {
  ...
  const handleExportHydroCAD = useCallback(() => {
    ...
    const { nodes, missingCnKeys } = aggregateOverlayForHydroCAD(
    ...
    if (missingCnKeys.size > 0) {
      addLog(
        'Overlay layer has polígonos sin CN calculado. Completa Land Cover y WSS antes de exportar.',
        'error'
      );
      return;
    }
