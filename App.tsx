    ...
    async (
      ...
      try {
        ...
        if (name === 'Soil Layer from Web Soil Survey') {
          ...
          if (!areaSymbol) {
            addLog(
              'No se encontró AREASYMBOL en la capa WSS. Se intentará obtener el HSG únicamente con los MUSYM.',
              'warn'
            );
          }
          if (symbols.length === 0) {
            ...
            try {
              const records = await fetchWssHsgRecords(symbols);
              ...
              const areaNote = areaSymbol ? ` (${areaSymbol})` : '';

              if (matchedFeatures === 0) {
                addLog(
                  `No se obtuvo HSG desde SDA para los ${symbols.length} símbolos consultados${areaNote}; revisa y completa manualmente.`,
                  'warn'
                );
              } else if (missingCount > 0) {
                addLog(
                  `Se autocompletó HSG para ${matchedFeatures} de ${totalFeatures} polígonos${areaNote}. Completa manualmente ${missingCount}.`
                );
              } else {
                addLog(
                  `HSG autoasignado para los ${totalFeatures} polígonos WSS${areaNote}. Revisa y ajusta si es necesario.`
                );
              }
            } catch (err) {
