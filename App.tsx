...
import { canonicalizeDischargePointName } from './utils/naming';
...
const App: React.FC = () => {
  ...
  const handleLayerAdded = useCallback((geojson: FeatureCollection, name: string, fieldMap?: Record<string, string>) => {
    ...
    if (name === 'Drainage Areas') {
      addLog(
        'Asigna cada área de drenaje a un Discharge Point usando el selector "Discharge Point #" en el mapa.'
      );
    }
    if (name === SUBAREA_LAYER_NAME) {
      addLog('Recuerda asociar cada Drainage Subarea a su Discharge Point en el campo PARENT_DA.');
    }
  ...
  const handleUpdateFeatureDaName = useCallback<UpdateDaNameFn>((layerId, featureIndex, nameVal) => {
    const canonical = canonicalizeDischargePointName(nameVal);
    updateFeatureProperty(layerId, featureIndex, 'DA_NAME', canonical);
    addLog(
      `Set Drainage Area discharge point for feature ${featureIndex} in ${layerId} to ${
        canonical || '--'
      }`
    );
  }, [addLog, updateFeatureProperty]);
  ...
  const runCompute = useCallback(async () => {
    ...
    if (da) {
      da.geojson.features.forEach(f => {
        ...
        const canonical = canonicalizeDischargePointName(raw);
        const normalized = raw === undefined || raw === null ? '' : String(raw).trim();
        const chosen = canonical || normalized;
        if (!chosen) return;
        const key = chosen.toLowerCase();
        if (!canonicalDaNames.has(key)) canonicalDaNames.set(key, canonical || chosen);
      });
    }
    ...
    if (sub) {
      sub.geojson.features.forEach(f => {
        ...
        const canonicalParent = canonicalizeDischargePointName(parentRaw);
        const parentName = canonicalParent || (parentRaw === undefined || parentRaw === null
          ? ''
    ...
    try {
      ...
      da.geojson.features.forEach(daFeature => {
        ...
        const daNameSource = canonicalizeDischargePointName(rawName) || candidateName;
        if (!daNameSource) return;
        const resolved = canonicalDaNames.get(daNameSource.toLowerCase()) || daNameSource;
        const daName = canonicalizeDischargePointName(resolved) || resolved;
