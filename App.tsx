...
const aggregateOverlayForHydroCAD = (features: Feature[]): HydroCADSubcatchment[] => {
  ...
  const ensureAggregate = (feature: Feature, index: number): SubareaAggregate => {
    const props = feature.properties || {};
    const rawName = (props as any)[SUBAREA_NAME_ATTR];
    const fallbackName = `Subarea ${index + 1}`;
    const name = resolveSubareaName(rawName, fallbackName);
    const key = getSubareaKey(rawName, fallbackName);
    ...
    if (existing) {
      ...
      if (index < existing.order) existing.order = index;
      const hasExplicit = rawName !== undefined && rawName !== null && String(rawName).trim() !== '';
      if (hasExplicit && existing.name !== name) {
        existing.name = name;
      }
      return existing;
    }
  ...
  features.forEach((feature, idx) => {
    ...
    const aggregate = ensureAggregate(feature, idx);
...
const App: React.FC = () => {
  ...
  const runCompute = useCallback(async () => {
    ...
    try {
      const { intersect: turfIntersect, area: turfArea } = await import('@turf/turf');
      ...
      if (overlay.length > 0) {
        ...
        try {
          ...
          if (subcatchments.length < expectedCount) {
            throw new Error(
              'No se generaron todos los subcatchments de HydroCAD según los polígonos de Overlay (Auto).'
            );
          }
