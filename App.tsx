    ...
    async (
      ...
      try {
        ...
        if (name === 'Soil Layer from Web Soil Survey') {
          ...
          if (symbols.length === 0) {
            addLog('No se detectaron MUSYM válidos en la capa WSS; los HSG permanecen vacíos.', 'warn');
          } else {
            if (!areaSymbol) {
              addLog(
                'No se encontró AREASYMBOL en la capa WSS; se consultarán los HSG sin ese filtro.',
                'warn'
              );
            }
            addLog(
              `Solicitando HSG desde SDA para ${symbols.length} símbolos${
            ...
            try {
              ...
              if (matchedFeatures === 0) {
                addLog(
                  `No se obtuvo HSG desde SDA para los ${symbols.length} símbolos consultados${
                    areaSymbol ? ` (${areaSymbol})` : ''
              ...
              const errorMessage = `No se pudo obtener el HSG desde SDA (${message}). Los HSG permanecen en blanco.`;
