...
import { normalizeDischargePointName } from './utils/dp';
...
const App: React.FC = () => {
    ...
    setLayers(prevLayers => {
      ...
      if (existing) {
        ...
        if (name === SUBAREA_LAYER_NAME) {
          addLog('Recuerda asociar cada Drainage Subarea a un Discharge Point # en el campo PARENT_DA.');
        }
        return updated;
      }
  ...
  const handlePreviewReady = useCallback((geojson: FeatureCollection, fileName: string, detectedName: string) => {
    ...
    if (detectedName === SUBAREA_LAYER_NAME) {
      addLog('Recuerda asociar cada Drainage Subarea a un Discharge Point # en el campo PARENT_DA.');
    }
  }, [addLog]);
  ...
  const handleUpdateFeatureDaName = useCallback<UpdateDaNameFn>((layerId, featureIndex, nameVal) => {
    const normalized = normalizeDischargePointName(nameVal);
    updateFeatureProperty(layerId, featureIndex, 'DA_NAME', normalized);
    addLog(
      `Set Discharge Point for drainage area feature ${featureIndex} in ${layerId} to ${normalized || '--'}`
    );
  }, [addLog, updateFeatureProperty]);
  ...
  const runCompute = useCallback(async () => {
    ...
    const attrDisplayNames: Record<string, string> = {
      [DA_NAME_ATTR]: 'Discharge Point #',
      [SUBAREA_PARENT_ATTR]: 'PARENT_DA (Discharge Point #)',
    };
    const registerMissing = (layerName: string, attr: string) => {
      if (!missingAttrs[layerName]) missingAttrs[layerName] = [];
      missingAttrs[layerName].push(attrDisplayNames[attr] ?? attr);
    };
    const checkAttrs = (
    ...
    if (da) {
      da.geojson.features.forEach(f => {
        ...
        const normalized = normalizeDischargePointName(rawText);
        const canonical = normalized || rawText;
        if (!canonical) return;
        const canonicalKey = canonical.toLowerCase();
        if (!canonicalDaNames.has(canonicalKey)) canonicalDaNames.set(canonicalKey, canonical);
        if (rawText) {
          const rawKey = rawText.toLowerCase();
          if (!canonicalDaNames.has(rawKey)) canonicalDaNames.set(rawKey, canonical);
        }
      });
    }
    ...
    if (sub) {
      sub.geojson.features.forEach(f => {
        ...
        const parentName = normalizeDischargePointName(parentRaw);
        if (!parentName) {
          orphanParents.add('(sin valor)');
          return;
        }
        const canonical = canonicalDaNames.get(parentName.toLowerCase()) ||
          canonicalDaNames.get(String(parentRaw ?? '').trim().toLowerCase());
        if (!canonical) {
    ...
    try {
      ...
      da.geojson.features.forEach(daFeature => {
        ...
        const rawName = daFeature.properties
          ...
          : null;
        const sanitizedName = rawName === undefined || rawName === null ? '' : String(rawName).trim();
        const normalizedName = normalizeDischargePointName(sanitizedName);
        const lookupKey = normalizedName ? normalizedName.toLowerCase() : sanitizedName.toLowerCase();
        const daName =
          (lookupKey && canonicalDaNames.get(lookupKey)) ||
          (sanitizedName && canonicalDaNames.get(sanitizedName.toLowerCase())) ||
          normalizedName ||
          sanitizedName;
        if (!daName) return;
