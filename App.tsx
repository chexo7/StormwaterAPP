...
const App: React.FC = () => {
  ...
  const project = useMemo(() => proj4('EPSG:4326', 'EPSG:3857'), []);
  ...
  const overlayForExport = useMemo(() => {
    const auto = layers.find(l => l.name === AUTO_OVERLAY_LAYER_NAME);
    if (auto && auto.geojson.features.length > 0) {
      return auto;
    }
    const manual = layers.find(l => l.name === 'Overlay');
    return manual && manual.geojson.features.length > 0 ? manual : null;
  }, [layers]);
  ...
  const exportEnabled = Boolean(overlayForExport);
  ...
  const handleSaveCnTable = useCallback(
    (records: CnRecord[]) => {
      ...
      addLog('Tabla de Curve Numbers actualizada para la sesión actual.');
      setCnTableOpen(false);
    },
    [addLog]
  );
  ...
  const runCompute = useCallback(async () => {
    ...
    const tasks: ComputeTask[] = [
      ...
      { id: 'overlay', name: 'HydroCAD SCS overlay analysis', status: 'pending' },
  ...
  const handleExportOverlayShapefile = useCallback(async () => {
    if (!overlayForExport) {
      addLog('No hay una capa de Overlay SCS disponible para exportar.', 'error');
      return;
    }

    const polygonFeatures = overlayForExport.geojson.features.filter(isPolygonLike);
    if (polygonFeatures.length === 0) {
      addLog('La capa de Overlay SCS no contiene polígonos válidos para exportar.', 'error');
      return;
    }
    ...
    try {
      ...
      const arrayBuffer = await shpwrite.zip(featureCollection, {
        outputType: 'arraybuffer',
        prj: WGS84_PRJ,
        filename: 'SCS_Overlay',
      });
      ...
      await Promise.all(
        Object.keys(sourceZip.files).map(async filename => {
          const content = await sourceZip.files[filename].async('arraybuffer');
          finalZip.file(filename, content);
        })
      );

      const dbfEntry = Object.keys(sourceZip.files).find(name =>
        name.toLowerCase().endsWith('.dbf')
      );
      if (dbfEntry) {
        const base = dbfEntry.replace(/\.dbf$/i, '');
        finalZip.file(`${base}.cpg`, 'UTF-8');
      }

      const blob = await finalZip.generateAsync({ type: 'blob' });
      const filename = `${(projectName || 'project')}_${projectVersion}_overlay_2d_scs.zip`;
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = filename;
      ...
      URL.revokeObjectURL(url);
      addLog(
        `Overlay SCS exportado como Shapefile 2D (${overlayForExport.name}).`
      );
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      addLog(`No se pudo exportar el Overlay SCS: ${message}`, 'error');
    }
  }, [overlayForExport, addLog, projectName, projectVersion]);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900 text-gray-100 font-sans">
      <Header
        ...
        onExport={handleExportOverlayShapefile}
