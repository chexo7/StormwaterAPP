...
import {
  SUBAREA_LAYER_NAME,
  PROCESSED_SUBAREA_LAYER_NAME,
  DA_NAME_ATTR,
  SUBAREA_NAME_ATTR,
  SUBAREA_PARENT_ATTR,
  COMPLEMENT_FLAG_ATTR,
  SUBAREA_AREA_ATTR,
...
const App: React.FC = () => {
  ...
  const handleUpdateFeatureDaName = useCallback<UpdateDaNameFn>((layerId, featureIndex, nameVal) => {
    const formatted = parseDischargePointId(nameVal) ?? (nameVal ? String(nameVal).trim().toUpperCase() : '');
    setLayers(prev =>
      prev.map(layer => {
        if (layer.id !== layerId) return layer;
        const features = [...layer.geojson.features];
        if (!features[featureIndex]) return layer;
        const feature = { ...features[featureIndex] };
        const properties = { ...(feature.properties || {}) };
        properties[DA_NAME_ATTR] = formatted;
        if (layer.name === SUBAREA_LAYER_NAME) {
          properties[SUBAREA_PARENT_ATTR] = formatted;
        }
  ...
  const runCompute = useCallback(async () => {
    ...
    if (da) {
      da.geojson.features.forEach(f => {
        ...
        const parsed = parseDischargePointId(raw);
        const normalized = parsed ?? String(raw).trim();
    ...
    if (sub) {
      sub.geojson.features.forEach(f => {
        ...
        const parsedParent = parseDischargePointId(parentRaw);
        const parentName = parsedParent ?? (parentRaw === undefined || parentRaw === null ? '' : String(parentRaw).trim());
        if (!parentName) {
