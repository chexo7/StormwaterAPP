...
const App: React.FC = () => {
  ...
  const hasDrainageAreas = layers.some(l => l.name === 'Drainage Areas');
  const exportHydroCADEnabled = computeSucceeded;
  const exportShapefilesEnabled = computeSucceeded;
  const exportSWMMEnabled = hasDrainageAreas || pipe3DEnabled;
  const exportEnabled =
    exportHydroCADEnabled || exportSWMMEnabled || exportShapefilesEnabled;
  ...
  const handleExportSWMM = useCallback(async () => {
    ...
    if (!daLayer && !(jLayer && pLayer)) {
      addLog('Drainage Areas layer not found and no pipe network', 'error');
      return;
    }
    ...
    let filteredPolygonLines: string[] = [];
    if (daLayer) {
      ...
      filteredPolygonLines = polygonLines.filter((l) =>
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-gray-100 font-sans">
      ...
      {exportModalOpen && (
        <ExportModal
          ...
          hydroCADEnabled={exportHydroCADEnabled}
          swmmEnabled={exportSWMMEnabled}
          shapefilesEnabled={exportShapefilesEnabled}
