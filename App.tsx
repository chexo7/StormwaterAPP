...
const App: React.FC = () => {
  ...
  const runCompute = useCallback(async () => {
    ...
    const tasks: ComputeTask[] = [
      { id: 'validateLod', name: 'Validate LOD polygon', status: 'pending' },
      { id: 'intersect1', name: 'Intersect LOD with other polygons', status: 'pending' },
    ];
    setComputeTasks([...tasks]);

    // Task 1: ensure LOD layer is a single polygon
    const lodValid =
      lod.geojson.features.length === 1 &&
      lod.geojson.features[0].geometry.type === 'Polygon';
    if (!lodValid) {
      tasks[0].status = 'error';
      setComputeTasks([...tasks]);
      addLog('LOD layer must contain a single Polygon feature', 'error');
      return;
    }
