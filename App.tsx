...
const App: React.FC = () => {
  ...
  const handleLayerAdded = useCallback((geojson: FeatureCollection, name: string, fieldMap?: Record<string, string>) => {
    ...
    if (name === 'Soil Layer from Web Soil Survey') {
      ...
      if (!daLayer) {
        const msg =
          'Carga las Drainage Areas y asigna sus Discharge Points antes de incorporar la capa del Web Soil Survey.';
        setError(msg);
        addLog(msg, 'error');
        return;
      }
      const missingDaIndexes: number[] = [];
      const daNames = new Set<string>();
      daLayer.geojson.features.forEach((feature, idx) => {
        const current = feature.properties ? (feature.properties as any)[DA_NAME_ATTR] : null;
        const formatted = formatDischargePointName(current);
        if (!formatted) {
          missingDaIndexes.push(idx + 1);
        } else {
          daNames.add(formatted);
        }
      });
      if (missingDaIndexes.length > 0) {
        const msg =
          'Completa los Discharge Points (DP-##) de todas las Drainage Areas antes de cargar la capa del Web Soil Survey.';
        setError(msg);
        addLog(msg, 'error');
        return;
      }
      ...
      if (!subLayer) {
        const msg =
          'Primero carga y asocia las Drainage Subareas a los Discharge Points antes de habilitar la capa del Web Soil Survey.';
        setError(msg);
        addLog(msg, 'error');
        return;
      }

      const subareaIssues: string[] = [];
      const missingSubNames: number[] = [];
      const missingParents: number[] = [];
      const unknownParents: Set<string> = new Set();
      subLayer.geojson.features.forEach((feature, idx) => {
        const props = feature.properties || {};
        const rawName = (props as any)[SUBAREA_NAME_ATTR];
        if (!rawName || !String(rawName).trim()) missingSubNames.push(idx + 1);
        const formattedParent = formatDischargePointName((props as any)[SUBAREA_PARENT_ATTR]);
        if (!formattedParent) {
          missingParents.push(idx + 1);
          return;
        }
        if (!daNames.has(formattedParent)) {
          unknownParents.add(formattedParent);
        }
      });

      if (missingSubNames.length > 0) {
        subareaIssues.push(
          `Asigna nombres "DRAINAGE AREA - #" a todas las Drainage Subareas antes de cargar la capa del Web Soil Survey (pendientes: ${missingSubNames.join(', ')}).`
        );
      }
      if (missingParents.length > 0) {
        subareaIssues.push(
          `Vincula cada Drainage Subarea a un Discharge Point válido (pendientes: ${missingParents.join(', ')}).`
        );
      }
      if (unknownParents.size > 0) {
        const unknownList = Array.from(unknownParents).join(', ');
        subareaIssues.push(
          `Algunas Drainage Subareas hacen referencia a Discharge Points que no existen en las Drainage Areas: ${unknownList}.`
        );
      }

      if (subareaIssues.length > 0) {
        const msg = subareaIssues.join(' ');
        setError(msg);
        addLog(msg, 'error');
        return;
      }
    ...
    if (name === 'Soil Layer from Web Soil Survey') {
      addLog('Selecciona manualmente el HSG (A, B, C o D) de cada polígono del Web Soil Survey usando el mapa.');
    }
