...
def get_hsg_bulk(symbols: list[str]) -> dict[str, str]:
    """Devuelve el HSG dominante para una lista de MUSYM.

    Usa una única consulta SQL para múltiples símbolos y aplica la misma regla de
    mayoría por frecuencia para determinar el HSG de cada uno.
    """
    cleaned = sorted({s.strip().upper() for s in symbols if s and s.strip()})
    if not cleaned:
        return {}

    placeholders = ",".join(f"'{sym.replace("'", "''")}'" for sym in cleaned)
    sql = f"""
    SELECT
      m.musym,
      (
    ...
    FROM mapunit m
    WHERE m.musym IN ({placeholders});
    """

    df = _run_sda_sql(sql)
    if df.empty or "musym" not in df.columns:
        return {sym: "" for sym in cleaned}

    result: dict[str, list[str]] = {sym: [] for sym in cleaned}
    for _, row in df.iterrows():
        musym = str(row.get("musym", "")).strip().upper()
        if musym not in result:
            continue
        cleaned_token = _clean_hsg_token(row.get("hsg"))
        if cleaned_token:
            result[musym].append(cleaned_token)

    final: dict[str, str] = {}
    for sym, tokens in result.items():
        if not tokens:
            final[sym] = ""
            continue
        counts = Counter(tokens)
        max_freq = max(counts.values())
        candidates = sorted([k for k, v in counts.items() if v == max_freq])
        final[sym] = candidates[0] if candidates else ""
