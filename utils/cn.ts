...
export const FALLBACK_CN_VALUES: CnValue[] = [
...
export async function loadCnValues(): Promise<CnValue[]> {
  try {
    const res = await fetch('/api/cn-values');
    if (res.ok) {
      return await res.json();
    }
    console.warn(`CN values request to /api/cn-values failed with status ${res.status}`);
  } catch (err) {
    console.warn('CN values request to /api/cn-values failed', err);
  }
  return FALLBACK_CN_VALUES as CnValue[];
}
