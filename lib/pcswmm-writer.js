...
function insertAfterSection(inp, section, block) {
...
function fmt(n, d = 3) { return Number.isFinite(n) ? n.toFixed(d) : '0'; }
function sanitizeId(s) { return s.replace(/[^\w\-]/g, '_'); }

function buildSUBCATCHMENTS(input) {
  const lines = [
    ...
    ...input.drainageAreas.map(a => {
      ...
      const area_ha = a.area_m2 ? (a.area_m2 / 10000) : 0;
      return [
        id,
...
function buildSUBAREAS(input) {
  ...
  for (const a of input.drainageAreas) {
    ...
    const sImp = a.sImperv_mm ?? 1.27;
    const sPerv = a.sPerv_mm ?? 2.54;
...
export { insertAfterSection, buildSUBCATCHMENTS, buildSUBAREAS, buildINFILTRATION, buildOUTFALLS };
