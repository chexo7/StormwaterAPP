  ...
  try {
    ...
    for (const chunk of chunkSymbols(symbols)) {
      ...
      table.forEach(row => {
        ...
        const musym = String(musymRaw ?? '')
          .trim()
          .toUpperCase();
        const muname = row?.muname ?? row?.MUNAME ?? '';
        const hsgToken = cleanHsgToken(row?.hsg ?? row?.HSG ?? row?.hydgrp ?? row?.HYDGRP);
        ...
        if (musym) {
          if (hsgToken) {
            addLog(`SDA devolvió HSG ${hsgToken} para MUSYM ${musym}.`);
          } else {
            addLog(
              `SDA no devolvió un HSG para MUSYM ${musym}; se mantendrá vacío hasta completar manualmente.`,
              'warn'
            );
          }
        }
