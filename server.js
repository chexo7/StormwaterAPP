...
const HSG_LETTER_REGEX = /[ABCD]/i;

const cleanHsgToken = value => {
  if (value === null || value === undefined) return '';
  const normalized = String(value).toUpperCase().trim();
  const match = normalized.match(HSG_LETTER_REGEX);
...
app.post('/api/wss-hsg', async (req, res) => {
  ...
  try {
    const results = [];
    for (const value of symbolsRaw) {
      const musym = value === null || value === undefined ? '' : String(value).trim();
      if (!musym) {
        results.push({ musym: '', hsg: '' });
        continue;
      }
      ...
      const tokens = table
        .map(row => cleanHsgToken(row?.hsg ?? row?.HSG ?? row?.hydgrp ?? row?.HYDGRP))
        .filter(Boolean);
      const hsg = selectDominantHsg(tokens);

      results.push({ musym: musym.toUpperCase(), hsg });
    }

    addLog(
      `Fetched HSG for ${results.length} MUSYM entries using SDA script logic.`
    );
    res.json({ results });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : 'Unknown error during SDA lookup';
    addLog(`Failed SDA lookup using script logic: ${message}`, 'error');
