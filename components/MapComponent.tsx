...
const ManagedGeoJsonLayer = ({
  ...
  const onEachFeature = (feature: GeoJSON.Feature, layer: Layer) => {
    if (feature.properties) {
      ...
      // Render all properties except special editable ones
...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  const [landCoverDescriptions, setLandCoverDescriptions] = useState<string[]>([]);

  useEffect(() => {
    loadLandCoverDescriptions().then(setLandCoverDescriptions);
  }, []);
        ...
        {layers.map((layer, index) => (
          <LayersControl.Overlay checked name={layer.name} key={layer.id}>
             <ManagedGeoJsonLayer
                ...
                landCoverOptions={landCoverDescriptions}
