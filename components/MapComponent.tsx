...
import { loadLandCoverValues } from '../utils/landcover';
...
const ManagedGeoJsonLayer = ({
  ...
  const onEachFeature = (feature: GeoJSON.Feature, layer: Layer) => {
    if (feature.properties) {
      ...
      // Editable land cover type
      if (layerName === 'Land Cover' && landCoverOptions.length > 0) {
        const lcRow = L.DomUtil.create('div', '', propsDiv);
        const lLabel = L.DomUtil.create('b', '', lcRow);
        lLabel.textContent = 'Land Cover: ';
        const select = L.DomUtil.create('select', '', lcRow) as HTMLSelectElement;
        select.title = 'Seleccionar tipo de Land Cover';
        select.style.marginLeft = '4px';
...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  useEffect(() => {
    loadLandCoverValues().then(setLandCoverOptions).catch(() => {});
  }, []);
