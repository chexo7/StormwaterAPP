...
const ManagedGeoJsonLayer = ({
  ...
  useEffect(() => {
    ...
    geoJsonRef.current.eachLayer((layer: any) => {
      ...
      if (isEditingLayer && editingFeatureIndex === idx) {
        layer.pm.enable({ snappable: true, snapDistance: 20, snapSegment: true, allowSelfIntersection: false });
      } else {
        layer.pm.disable();
      }
    });
  ...
  // When geometry is edited with Geoman, propagate changes up
  useEffect(() => {
...
const GeomanControls = ({
  ...
  useEffect(() => {
    ...
    // enable snapping/editing on existing layers
    layer.eachLayer((l: any) => l.pm.enable({ snappable: true }));
    ...
    // validation for overlaps on create and edit
    const validateOverlap = (e: any) => {
    ...
    map.on('pm:create', validateOverlap);
    map.on('pm:edit', validateOverlap);

    return () => {
      map.off('pm:create', handleCreate);
      map.off('pm:remove', handleRemove);
      map.off('pm:edit', handleEdit);
      map.off('pm:create', validateOverlap);
      map.off('pm:edit', validateOverlap);
