    ...
    if (feature.properties) {
      ...
      Object.entries(feature.properties).forEach(([k, v]) => {
        ...
        if (k === 'HSG') {
          ...
          label.innerHTML = `<b>${k}:</b>`;
          const select = L.DomUtil.create('select', '', row) as HTMLSelectElement;
          select.className = 'ml-1 px-1 py-0.5 bg-gray-700 text-yellow-300 border border-yellow-500 rounded';
          ['A', 'B', 'C', 'D'].forEach(val => {
            const opt = L.DomUtil.create('option', '', select) as HTMLOptionElement;
            opt.value = val;
            opt.textContent = val;
            if (String(v) === val) opt.selected = true;
          });
          select.addEventListener('change', (e) => {
            ...
            feature.properties![k] = newVal as any;
          });
        } else {
